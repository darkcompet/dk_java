...
/**
 * This maps int vs boolean.
 * We make this for Java (from Android).
 */
public class DkIntBooleanArrayMap implements Cloneable {
	private int size;
	private int[] keys;
	private boolean[] values;

	public DkIntBooleanArrayMap() {
		this(10);
	}
	 ...
	 */
	public DkIntBooleanArrayMap(int initialCapacity) {
		if (initialCapacity <= 0) {
			keys = MyEmptyArray.INT;
			values = MyEmptyArray.BOOLEAN;
		}
		else {
	...
	/**
	 * Gets the int mapped from the specified key, or `false` if no such mapping has been made.
	 */
	...
	public boolean get(int key, boolean valueIfKeyNotFound) {
		...
		return index < 0 ? valueIfKeyNotFound : (boolean) values[index];
	}
	...
	@NonNull
	@Override
	public DkIntBooleanArrayMap clone() {
		DkIntBooleanArrayMap clone;
		try {
			clone = (DkIntBooleanArrayMap) super.clone();

...
/**
 * This maps int vs object.
 * We make this for Java (from Android).
 *
 * O: value type.
 */
@SuppressWarnings("unchecked")
public class DkIntObjectArrayMap<O> implements Cloneable {
	private int size;
	private int[] keys;
	 ...
	 */
	public DkIntObjectArrayMap(int initialCapacity) {
		if (initialCapacity <= 0) {
			keys = MyEmptyArray.INT;
			values = MyEmptyArray.OBJECT;
		}
		else {
	...
	/**
	 * Gets the int mapped from the specified key, or `null` if no such mapping has been made.
	 */
	 ...
	 */
	public O get(int key, O valueIfKeyNotFound) {
	 ...
	 */
	public int indexOfValue(O value) {
	 ...
	 */
	public void append(int key, O value) {
	...
	@NonNull
	@Override
	public DkIntObjectArrayMap<O> clone() {
		DkIntObjectArrayMap<O> clone;
		try {
			clone = (DkIntObjectArrayMap<O>) super.clone();
